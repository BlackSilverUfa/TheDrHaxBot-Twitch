...
* `cooldown [имя] [канал] [польз.] [действ.]` - установить временнЫе ограничения на использование команды;
  * Параметры:
    ...
    * `[действ.]` - действие при превышении указанной частоты сообщений;
      * `remove` (по умолчанию) - удалить сообщение;
      * `ignore` - не отвечать на сообщение;
  * Значения по умолчанию: 10 / 0
  * Допускается подстановка `-` для сохранения параметров неизменными:
    * `cooldown [имя] - - ignore` - поле `[действ.]` получит значение `ignore`, а остальные не изменятся;

* `plugin [имя] [параметры]` - настройка разных плагинов;
  * `context` - управление историей чата. В данный момент используется для ИИ.
    * `clear` - очистить историю полностью;
    * `remove <regex>` - удалить сообщения, подходящие под регулярное выражение. Можно использовать просто какое-то слово, на котором ИИ помешался.
    * `restore <regex>` - отменяет удаление сообщений, подходящих под регулярное выражение. Не действует на сообщения, удалённые модерацией чата или пользователем.
  ...
  * `silent` - включает режим тихих ответов от бота;
    * `on` или `true` - включено;
    * `off` или `false` - отключено;
    * **Важно**: Применимо в данный момент только в Telegram;
  * `chroot [канал]` - эмулирует нахождение на другом канале;
    * **Важно**: Только для администратора бота;
    * Вместо ID канала можно указать `-`, чтобы отключить эту функцию;
