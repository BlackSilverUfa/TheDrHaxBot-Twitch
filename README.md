...
* `replace [имя] [оригинал] / [замена]` - заменяет текст внутри ответа команды или один из алиасов. Например, так можно исправить одно или несколько слов в большой команде (добавить пункт в правила и т.п.), или подправить алиас без его удаления;
  * Если `[оригинал]` встречается одновременно в тексте и в алиасе, то будет выдано сообщение о конфликте. Способы разрешения конфликтов:
    * Использовать более строгое правило в `[оригинале]`;
    * Заменить текст ответа команды напрямую через `update`;
    * Пересоздать алиас через `remove` и `add alias`;
