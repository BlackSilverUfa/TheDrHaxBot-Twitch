...
* `add [тип] [имя] [значение]` - добавляет новую команду. У разных типов команд используются разные `[значения]`:
  ...
  * `function [имя] [команда]` - создаёт команду, которая вызывает другую команду;
    * **ВАЖНО**: команды выполняются с привилегиями модератора, т.е. если вы сделаете функцию с командой `!tdhbot {query}`, то любой пользователь сможет управлять ботом. НЕ НАДО ТАК ДЕЛАТЬ!
